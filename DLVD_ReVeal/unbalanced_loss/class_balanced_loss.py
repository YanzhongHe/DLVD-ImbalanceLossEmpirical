...
class Loss(torch.nn.Module):
    ...
    def forward(
        ...
        if self.loss_type == "focal_loss":
        ...
        elif self.loss_type == "focal_loss_alpha":
            cb_loss = focal_loss(logits, labels_one_hot, alpha=self.alpha, gamma=self.fl_gamma)        
        elif self.loss_type == "focal_loss_0":
            
            
            # Number of negative samples
            negative_samples = self.samples_per_class[0]
            positive_samples = self.samples_per_class[1]

            # total sample number
            total_samples = sum(self.samples_per_class)

            # Calculate the number of negative samples divided by the total number of samples
            negative_ratio = negative_samples / total_samples
            positive_ratio = positive_samples / total_samples
                                
            # Calculate the weight based on the proportion of negative samples and the proportion of positive samples
            neg_weight = 1.0 / negative_ratio
            pos_weight = 1.0 / positive_ratio

            # Create label_weight tensor, assuming batch_size is args.train_batch_size
            batch_size = logits.size(0)
            label_weight = torch.zeros((batch_size, 2), device=logits.device)  # 2表示两个类别

            # Set the weight of all samples
            ...
            cb_loss = focal_loss(logits, labels_one_hot, alpha=label_weight)
